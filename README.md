...
- **整页翻译**：翻译当前页面可见文本
- **右侧悬浮球**：固定在页面右侧中间，可翻译此页 / 恢复原文 / 切换目标语言 / 双语对照开关
- **双语对照**：开启后保留原文；段落按整段翻译（不再拆碎），短标签译文靠右、长段译文在下；关闭则为替换原文模式
- **智能右键菜单**：未翻译时显示「翻译此页面」，已翻译后变为「恢复原文」
- **选区粉色圆点**：选中文字后出现小圆点，悬停预览译文并支持朗读原文/译文
- **输入框互译**：聚焦输入框时显示「译」按钮，按目标语言智能互译
